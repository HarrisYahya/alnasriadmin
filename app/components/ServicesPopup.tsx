...
export default function ServicesPopup({
  ...
  return (
    <div className={`fixed inset-0 flex items-center justify-center z-50 p-4 ${
      ...
      <div className={`rounded-3xl shadow-2xl w-full max-w-lg p-5 md:p-7 animate-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto ${
        neonMode
        ...
        <div className={`flex justify-between items-center pb-3 mb-4 ${
          ...
          <h2 className={`text-xl md:text-2xl font-light ${neonMode ? "text-cyan-300" : "text-gray-800"}`}>
            ✦ <span className={neonMode ? "font-semibold text-transparent bg-clip-text bg-linear-to-r from-cyan-400 to-purple-400" : "font-semibold text-amber-600"}>Select Services</span>
          </h2>
          <button
        ...
        <div className="space-y-3 max-h-80 overflow-y-auto pr-1 custom-scroll">
          {servicesList.map((s: { name: string; fee: number }) => (
            <label
              key={s.name}
              className={`flex flex-wrap items-center justify-between p-3 rounded-xl border cursor-pointer transition-all ${
                neonMode
            ...
            >
              <div className="flex items-center gap-3">
                <input
                  ...
                  className={`w-5 h-5 rounded ${neonMode ? "accent-cyan-500" : "accent-amber-500"}`}
                />
                <span className={`font-medium ${neonMode ? "text-cyan-100" : "text-gray-700"}`}>{s.name}</span>
              </div>
              <span className={`font-mono ${neonMode ? "text-purple-400" : "text-amber-600"}`}>${s.fee}</span>
            </label>
          ))}
        </div>
      ...
      <style jsx>{`
        ...
        .custom-scroll::-webkit-scrollbar-thumb {
          ...
          border-radius: 10px;
          ${neonMode ? "box-shadow: 0 0 5px #0ff;" : "box-shadow: 0 0 2px #f59e0b;"}
        }
